    ...
    async def _get_samples_from_queue(self) -> tuple[None, None] | tuple[int, Any]:
        ...
        if self.config.trainer.balance_batch:
            batch = assemble_batch_from_rollout_samples(
                queue_samples, self.tokenizer, self.config, self._balance_batch, processor=self.processor
            )
        else:
            batch = assemble_batch_from_rollout_samples(
                queue_samples, self.tokenizer, self.config, None, processor=self.processor
            )
