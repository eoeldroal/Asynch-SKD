...
### carryover에 대한 step 간 sticky extension

현재 구현은 이 sticky routing을 **carryover sample에 한해 step 간으로 연장하는 기능**을 지원한다.

- carryover partial은 `teacher_replica_id`를 상태에 함께 저장한다
- `AsyncSkdAgentLoopManager`는 `sample_id -> real teacher server_id`와 `sample_id -> teacher_routing_key` ledger를 직접 유지한다
- 다음 step의 resumed carryover current work는 이 ledger를 기준으로 **같은 routing_key 내부에서 hard pin** 된다
- 새 base sample은 carryover가 먼저 점유한 teacher load를 반영한 뒤에만 rebalance된다

기능이 켜져 있을 때 teacher 쪽 실행 정책은:

1. carryover sample은 가능한 한 이전 teacher pool의 같은 real server로 다시 보낸다
2. base sample은 그 전제 위에서 같은 teacher pool 내부 least-loaded server로 채운다

이 정책은 student에는 그대로 적용되지 않는다. student는 actor update 과정에서 sleep / residency transition을 거치므로, teacher와 달리 step 간 KV reuse의 직접 대상이 아니다.

또 이 기능은 현재 config로 끌 수 있다. `actor_rollout_ref.rollout.agent.async_skd_teacher_sticky_carryover=False`면 carryover도 fresh처럼 현재 step에서 다시 배치되고, teacher pin 관련 metric은 사실상 비활성 상태로 해석해야 한다.

여기서 설계상 중요한 교정이 있었다. 초기 확장안은 planner 내부에서 `teacher-replica-{idx}` 같은 placeholder 이름을 만들 수 있었는데, 이건 실제 `GlobalRequestLoadBalancer`가 아는 server ID와 다를 수 있어 bind 단계에서 깨진다. 따라서 현재 source of truth는:

- teacher manager가 노출하는 `server_ids_by_routing_key()`
- carryover partial에 저장된 `teacher_replica_id` / `teacher_routing_key`

두 가지다. planner는 이 실제 server ID 집합 밖의 pin을 hard pin으로 인정하지 않고, 해당 routing key pool 안에서 fallback assign을 수행한다.

중요한 점은, 이 확장 역시 **live KV handle resume**은 아니라는 것이다. 현재 구현은 여전히:

- same-replica routing
- full-prefix fresh request
- radix/prefix cache 재매치

구조를 유지한다. 달라진 것은 sticky 범위가 “한 step 내부”를 넘어 carryover lifetime까지 확장되었다는 점이다.
...
여기서 끝이 아니다. validation이 async manager를 재사용하더라도 training source state를 건드리면 안 되므로, trainer는 `_validate()` 동안 `AsyncSkdDataSource`를 manager에서 잠깐 떼고 `finally`에서 복원한다. 즉 validation은:

- student-only generation
- training future sample reservation 없음
- training promoted/carryover ledger writeback 없음

이어야 한다. 이 격리가 없으면 validation이 training carryover를 몰래 늘리는 leakage bug가 생긴다.
